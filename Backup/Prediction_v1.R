data_model <- read.csv("data_model2.csv")
data_fighter <- read.csv("fighter_data.csv")
control <- trainControl(method="cv", number=10)
metric <- "Accuracy"
set.seed(123)
split = sample.split(data_model, SplitRatio = 0.80)
training_set = subset(data_model, split==TRUE)
test_set = subset(data_model, split==FALSE)
fit.lda <- train(Winner~., data=data_model, method="lda", metric=metric, trControl=control)
...
fighter_choosen_data=data.frame(
  ...
  diff_KD                     =blue$avg_KD - red$avd_KD,               
  diff_PASS                   =blue$avg_PASS - red$avd_PASS,
  diff_REV                    =blue$avg_REV - red$avd_REV,
