...
data_clean2 <- data_clean %>% select(-contains("B_"),-contains("R_"))

#Save in csv file
write.csv(data_clean2,"data_model2.csv", row.names = FALSE)
...
set.seed(123)
split = sample.split(data_clean2, SplitRatio = 0.80)
training_set = subset(data_clean2, split==TRUE)
test_set = subset(data_clean2, split==FALSE)

#a) linear algorithms
set.seed(123)
fit.lda <- train(Winner~., data=data_clean2, method="lda", metric=metric, trControl=control)
# b) nonlinear algorithms
# CART
set.seed(123)
fit.cart <- train(Winner~., data=data_clean2, method="rpart", metric=metric, trControl=control)
# kNN
set.seed(123)
fit.knn <- train(Winner~., data=data_clean2, method="knn", metric=metric, trControl=control)
# c) advanced algorithms
# SVM
set.seed(123)
fit.svm <- train(Winner~., data=data_clean2, method="svmRadial", metric=metric, trControl=control)
